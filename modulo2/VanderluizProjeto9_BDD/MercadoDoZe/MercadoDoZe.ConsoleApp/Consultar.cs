...
namespace MercadoDoZe.ConsoleApp
{
    public class Consultar
    {
        ...
        public static void ConsultarRegistroPedido(Pedido pedido, PedidoDAO pedidoDAO)
        {
            var opcaoStr = String.Empty;
            ...
            Console.WriteLine("-----------------------------------------------------");
            Console.WriteLine("|    [1] Consultar todos os PEDIDOS cadastrados.    |");
            Console.WriteLine("|    [2] Consultar PEDIDOS por ID.                  |");            
            Console.WriteLine("-----------------------------------------------------");           
            opcaoStr = Console.ReadLine();
            ...
            switch (opcaoStr)
            {
                case "1":
                    Console.Clear();
                    listaPedidos = pedidoDAO.BuscarTodos();
                    Imprimir(listaPedidos);                    
                    Console.ForegroundColor = ConsoleColor.Green;
                    System.Console.WriteLine("Precione enter para continuar!");                        
                    Console.ForegroundColor = ConsoleColor.White; 
                    break;
                case "2":
                    Console.Clear();
                    Console.WriteLine("Digite um ID para consulta: (ex: 2)");
                    var id = int.Parse(Console.ReadLine());
                    pedido = pedidoDAO.BuscarPorId(id);
                    if (pedido.Id != 0)
                    {
                       System.Console.WriteLine(pedido.ToString());   
                    }                                      
                    break;                
                default:
                    Console.ForegroundColor = ConsoleColor.Green;
            ...
            void Imprimir(List<Pedido> listaPedidos)
            {
